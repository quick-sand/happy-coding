...
def admin(request):
    if request.method == 'GET':
        ...
        if admin_uid == '111' and admin_pwd == '222':
            # 管理员登录成功
            response['status'] = True
            request.session['admin_uid'] = 'liuxiao'
            return HttpResponse(json.dumps(response))
        else:
            response['msg'] = '用户名或者密码错误'
            return HttpResponse(json.dumps(response))
...
def enrol(request):
    ...
    if request.method == 'POST':
        uid = request.POST.get('uid')
        pwd = request.POST.get('pwd')
        pwd_certain = request.POST.get('pwd_certain')
        type = request.POST.get('type')
        response = {'msg': '', 'status': False}
        ...
        if len(uid) == 0:
            response['msg'] = '请输入用户名'
            return HttpResponse(json.dumps(response))
        elif len(models.User.objects.filter(uid=uid)) != 0:
            response['msg'] = '改用户已被创建'
            return HttpResponse(json.dumps(response))
        elif pwd != pwd_certain:
            response['msg'] = '请核对两次密码输入是否一致'
            return HttpResponse(json.dumps(response))
        else:
            if type == 'enrol':
                models.User.objects.create(uid=uid, password=pwd)
...
def edit_pwd(request):
    ...
    if request.method == 'POST':
        uid = request.session.get('uid')
        pre_pwd = models.User.objects.filter(uid=uid).values('password').first()
        old = request.POST.get('oldpwd')
        new1 = request.POST.get('pwd')
        new2 = request.POST.get('pwd2')
        response = {'msg': '', 'status': False}

        if old != pre_pwd['password']:
            response['msg'] = '请确认原密码输入是否正确'
            return HttpResponse(json.dumps(response))
        elif new1 != new2:
            response['msg'] = '请核对两次密码输入是否一致'
            return HttpResponse(json.dumps(response))
